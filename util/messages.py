...
def invoke_graph(
    ...
    **stream_kwargs: Any,
    ...
    Args:
        graph (CompiledStateGraph): 실행할 컴파일된 그래프 객체
        inputs (dict): 그래프에 전달할 입력값 딕셔너리
        config (RunnableConfig, optional): 실행 설정. 기본값은 None
        context (Any, optional): 그래프 실행을 위한 정적 컨텍스트. 기본값은 None
        node_names (List[str], optional): 출력할 노드 이름 목록. 기본값은 빈 리스트
        ...
        **stream_kwargs: ``CompiledStateGraph.stream`` 에 전달할 추가 인자

    Returns:
        마지막 전체 상태(``stream_mode="values"`` 스냅샷). 스트림이 비면 ``None``.
    """
    ...
    # updates로 노드별 출력, values로 최종 상태(한 번의 실행으로 수집)
